...
class HelpActionShowDocumentationTest {
    ...
    static Stream<Arguments> shouldReturnINodeJSEnvironment() {
        return Stream.of(Arguments.of(mock(INodeJSEnvironment.class), mock(Project.class)), Arguments.of(null, null));
    }
    ...
    class GetNodeJSEnvironment {
       ...
       /* @ParameterizedTest
        ...
        void shouldHandleProjectNotNull(INodeJSEnvironment pExpected, Project pProject) {
            // Arrange
            try (MockedStatic<INodeJSProvider> mockedStatic = mockStatic(INodeJSProvider.class)) {
                INodeJSProvider nodeJSProviderMock = mock(INodeJSProvider.class);
                mockedStatic.when(() -> INodeJSProvider.findInstance(pProject)).thenReturn(Optional.of(nodeJSProviderMock));

                Observable<INodeJSEnvironment> observableMock = Observable.just(pExpected);
                when(nodeJSProviderMock.current()).thenReturn(observableMock);
